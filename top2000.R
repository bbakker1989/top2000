...
# read in data
top2000 <- read.delim("top2000.txt")
top2000 <- top2000 %>%
  as_tibble() %>%
  mutate(across(.cols = 5:ncol(top2000), .fns = function(x) {gsub(x = x, pattern = "X", 
                                                       replacement = NA)}),
         across(.cols = 5:ncol(top2000), .fns = function(x) {as.integer(x)}),
...
p1 <- decade_count %>%
  ...
  ggrepel::geom_text_repel(data = decade_count %>%
                             filter(editie == max(editie)), xlim = c(max_year, max_year),
                           aes(label = decennium), hjust = -0.1, show.legend = FALSE) +
  expand_limits(x = c(min_year, max_year+2)) +
  scale_x_continuous(breaks = seq(min_year, max_year)) +
...
p2 <- top_artiest_plot %>%
  ...
  labs(x = "Editie", y = "Aantal nummers", col = "Artiest",
       title = "Aantal nummers van de top 11 artiesten in de lijst",
       subtitle = "Artiesten met de meeste nummers in 2025 worden getoond") +
  expand_limits(x = c(min_year, max_year)) +
  scale_x_continuous(breaks = seq(min_year, max_year)) +
  scale_color_manual(values = scales::hue_pal()(nrow(artiest_levels))) +
  top2000_theme() +
...
p3 <- top2000_tidy %>%
  na.omit() %>%
  filter(editie %in% ceiling(seq(from = min_year, to = max_year, length.out = 6))) %>%
  ...
  coord_cartesian(x = c(1950, max_year)) +
  top2000_theme() +
  theme(legend.position = "bottom")
p3
...
top2000_mat <- top2000 %>%
  ...
  mutate(across(.cols = 1:27, .fns = function(x) {ifelse(is.na(x), 5000, x)})) %>%
  as.matrix()
...
p5 <- top2000_tidy %>%
  na.omit() %>%
  filter(jaar >= 1950, 
         editie %in% ceiling(seq(from = min_year, to = max_year, length.out = 6))) %>%
  ...
  labs(x = "Jaar van uitgave", y = "Positie",
       ...
       subtitle = "Pearsons correlatieco?fficient en p-waarden worden weergegeven") +
  top2000_theme()
p5
...
ggsave(p0, file = "plot0.png", width = 20, height = 12)
